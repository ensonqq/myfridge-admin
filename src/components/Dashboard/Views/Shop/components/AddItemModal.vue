  ...
  <ValidationObserver v-slot="{ handleSubmit }">
    <form @submit.prevent="handleSubmit(save)">
      ...
      <modal :show.sync="addItemModal" header Classes="justify-content-center">
        <h4 slot="header" class="title title-up">增加物品到組合</h4>
        <div>
          <div class="row">
            <div class="col-12">
...
export default {
  ...
  name       : "AddItemModal",
  data       : () => ({
    item         : { product : null, quantity : null },
    addItemModal : false,
    resolve      : null,
    reject       : null,
    products     : []
  ...
  methods    : {
    ...
    async save () {
      try {
        ...
        const item = { product, quantity : this.item.quantity }
        this.resolve(item)
      } catch (error) {
    ...
    reset () {
      ...
      this.item = { category : null, quantity : null }
    }
  }
}
</script>
