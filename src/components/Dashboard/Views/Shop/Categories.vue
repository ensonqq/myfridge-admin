<template>
  <div class="card">
    ...
    <div class="card-body row">
      <div class="col-12 row">
        ...
        <div class="col-lg-4 col-12">
          <div class="pull-right d-flex">
            <div class="mr-3">
              <el-button @click="openCategoryDetail()">＋建立新類別</el-button>
            </div>
      ...
      <div class="col-sm-12">
        <el-table v-loading="loading"
          ...
          <el-table-column prop="buyXGetYFree"
                           label="買X送Y" min-width="200">
            <template #default="scope">
              <el-switch v-model="scope.row.buyXGetYFreeEnabled"
                         @change="changeBuyXGetYFreeEnabled(scope.row.id, $event)"
                         active-color="#13ce66"
                         inactive-color="#ff4949">
              </el-switch>
              {{ scope.row.buyXGetYFreeEnabled ? '啟用' : '停用' }}
              <br>
              <template v-if="scope.row.buyXGetYFree[0]">
                <div v-for="(item, index) in scope.row.buyXGetYFree" :key="index" :class="{'text-gray': !scope.row.buyXGetYFreeEnabled}">
                  Tier{{ index + 1 }}: 買{{ item.x }}送{{ item.y }}
                  <hr class="mt-0 mb-0" v-if="index < scope.row.buyXGetYFree.length - 1">
                </div>
              </template>
              <template v-else>
                <span class="text-danger" :class="{'text-gray': !scope.row.buyXGetYFreeEnabled}">-未有買X送Y折扣</span>
              </template>
            </template>
          </el-table-column>
          ...
          <el-table-column class-name="action-buttons td-actions" label="">
            <template #default="scope">
              <p-button type="success" icon @click.stop="openCategoryDetail(scope.row.id)">
    ...
    <CategoryDetail ref="CategoryDetail"></CategoryDetail>
  </div>
</template>
...
export default {
  ...
  methods : {
    ...
    async openCategoryDetail (categoryId) {
      const result = await this.$refs.CategoryDetail.open(categoryId)
      if (result) {
        //DO STH
        this.getCategories()
      }
    ...
    async changePublish (categoryId, val) {
      const loading = new Loading.service({ fullscreen : false });
      try {
        const result = await this.$api.patch('/v1/categories/' + categoryId, { publish : val })
        if (result) {
    ...
    async changeDiscountTierEnabled (categoryId, val) {
      const loading = new Loading.service({ fullscreen : false });
      try {
        const result = await this.$api.patch('/v1/categories/' + categoryId, { discountTierEnabled : val })
        if (result) {
          this.$notify({ title : '成功', message : '更新成功', type : 'success', })
        }
      } catch (e) {
    ...
    async changeBuyXGetYFreeEnabled (categoryId, val) {
      const loading = new Loading.service({ fullscreen : false });
      try {
        const result = await this.$api.patch('/v1/categories/' + categoryId, { buyXGetYFreeEnabled : val })
        if (result) {
          this.$notify({ title : '成功', message : '更新成功', type : 'success', })
        }
      } catch (e) {
