<template>
  <div class="card">
    ...
    <div class="card-body">
      ...
      <div class="row mt-2">
        <div class="col-lg-8 col-12 row">
          ...
          <div class="col-lg-2 col-12" v-show="$route.name === 'OrdersByDeliveryDateV2' || nav === 'deliveryDay'">
            <fg-input label="收貨日">
          ...
          <div class="col-lg-8 col-12 row align-items-center print-controls" v-show="$route.name === 'OrdersByDeliveryDateV2'">
            <div class="mt-3">
              <el-button @click="printSFOrders('allUnprinted')">列印所有未印({{ getUnprintedCnt }})</el-button>
            </div>
            <div class="mt-3">
              <el-button @click="printSFOrders('selected')">列印已選({{ multipleSelection.length }})</el-button>
            </div>
            <!-- removed on 14 Apr -->
            <!--<div class="mt-3">
      ...
      <div class="col-sm-12">
        <el-table v-loading="loading"
          ...
          <el-table-column label="" class-name="action-buttons td-actions" min-width="100">
            <template slot-scope="props">
              <div class="d-flex">
                <p-button type="success" icon @click.stop="openDetail(props.row.orderNumber)">
                  <i class="fa fa-edit" aria-hidden="true"></i>
                </p-button>
                <p-button type="warning" icon @click.stop="resendReceipt(props.row.orderNumber)">
                  <i class="fa fa-envelope" aria-hidden="true"></i>
                </p-button>
              </div>
            </template>
          </el-table-column>
          <el-table-column label="訂單號碼" min-width="280">
            <template #default="scope">
              <div class="d-flex" :class="{'text-danger': scope.row.flag}">
                <div>{{ scope.row.orderNumber }}{{ scope.row.flag ? '⚠️' : '' }}</div>
                <div v-if="scope.row.printed" class="text-danger">(已印)</div>
                <p-button v-if="scope.row.delivery.billNo" type="info" size="sm" :class="'ml-2 order-routes-'+scope.row.id+'-button'" icon @click.stop="showOrderRoute(scope.row.orderNumber, scope.row.accessCode)">
          ...
          <el-table-column prop="email"
            ...
            <template #default="scope">
              <div v-if="scope.row.createdBy" class="text-success" @click.stop="openUserProfile(scope.row.createdBy.id)">
                ...
                <span v-if="scope.row.createdBy.vipTier === 'diamond'" style="color: #431ec8">(鑽)</span>
          ...
          <el-table-column label="訂單物品" min-width="400">
            <template #default="scope">
              <template>
                ...
                <div :class="'hidden order-items-'+scope.row.id">
                  <div v-for="item in scope.row.items">
                    <div style="display: flex; flex-wrap: nowrap;">
                      ...
                      <div>
                        {{ item.product.name.zh }} (${{ item.unitPrice }})
                        <template v-if="item.addons">
                          <br><span style="margin-left: 5px; font-size: 13px;">- {{ item.addons.name.zh }} (${{ item.addons.price }})</span>
                        </template>
                      </div>
                    </div>
                  </div>
          ...
          <el-table-column label="贈品" min-width="220">
            <template #default="scope">
              <div>
                利是:
                <span v-if="scope.row.prize">
                  <span v-if="scope.row.prize.type === 'cash'">-${{ scope.row.prize.amount }}利是</span>
                  <span v-else>{{ scope.row.prize.product.name.zh }}x{{ scope.row.prize.amount }}件</span>
                </span>
                <span v-else>--</span>
              </div>
              ...
              <div style="border-top: 1px solid #eee">
                禮品:
                <span v-if="scope.row.gift">
                  <span v-if="scope.row.gift.type === 'other'">{{ scope.row.gift.giftRef.name.zh }} x{{ scope.row.gift.amount }}件</span>
                  <span v-else>{{ scope.row.gift.product.name.zh }} x{{ scope.row.gift.amount }}件</span>
                </span>
                <span v-else>--</span>
              </div>
            </template>
          ...
          <el-table-column label="多買多減Tier" min-width="200">
            <template #default="scope">
              <div>
                <div v-if="scope.row.autoDiscountTiersAdopted.length || scope.row.buyXGetYFreeAdopted.length">
                  <template v-if="scope.row.autoDiscountTiersAdopted.length">
                    ...
                    <hr style="padding: 1px 0 1px 0; margin: 0">
                  </template>
                  <template v-if="scope.row.buyXGetYFreeAdopted.length">
                    <div v-for="autoTier in scope.row.buyXGetYFreeAdopted">
                      {{ autoTier.desc }} (共送{{ autoTier.freeItems.length }}件)
                    </div>
                    <b>-${{ scope.row.buyXGetYFreeDiscountTotal * -1 }}</b>
                  </template>
                </div>
                <template v-else>--</template>
              </div>
    ...
    <OrderDetail @openUserProfile="openUserProfile" ref="OrderDetail"></OrderDetail>
...
import printOrders_SF                from "@/util/printOrders_SF";
...
export default {
      ...
      this.result.results.forEach(order => {
        order.items.forEach(item => {
          if (item.product.isBundled) {
            ...
            if (summary[item.product.id]) {
              summary[item.product.id].quantity += item.quantity
            } else {
              // console.log(item.product.name.zh)
            }
          }
        })
      })
  ...
  watch      : {
    ...
    datePicker (val) {
      ...
      if (val) {
        ...
        this.filters.createdAtFrom = moment(val).startOf('day').format()
        this.filters.createdAtTo = moment(val).endOf('day').format()
      }
    },
    date (val) {
  ...
  methods : {
    ...
    async showOrderRoute (orderNumber, accessCode) {
      const result = await this.$refs.OrderRoute.open(orderNumber, accessCode)
      if (result) {
        //DO STH
      }
    },
    ...
    async openDetail (orderNumber) {
      ...
      if (result) {
        //DO STH
        if (result) {
          for (let i in this.result.results) {
            const oldOrderData = this.result.results[i]
            if (oldOrderData.orderNumber === result.orderNumber) {
              this.result.results[i] = result
            }
          }
        }
      }
    ...
    async printSFOrders (type) {
      const haveFlag = this.result.results.filter(order => order.flag)
      if (haveFlag.length) {
        const confirmation = await this.$refs.ConfirmDialog.open('有問題訂單', '有訂單有問題，請檢查清楚有警告的訂單？')
      ...
      const loading = Loading.service({
        fullscreen  : true,
        lock        : true,
        customClass : 'sf-print-spinner',
        text        : '所需時間較長，請不要關閉視窗！正在批量進行順豐下單及獲取印單中...',
