  ...
  <div class="card">
    <div class="card-header">
      <h5 class="card-title">{{ $route.meta.title }}
        <el-button @click="getPromotions" class="pull-right"><i class="fa fa-refresh"></i></el-button>
      </h5>
    </div>
    <div class="card-body row">
      <div class="col-12 row">
        ...
        <div class="col-lg-4 col-12">
          <div class="pull-right d-flex">
            <div class="mr-3">
              <el-button @click="openPromotionDetail()">＋建立新網站優惠Banner</el-button>
            </div>
            <fg-input class="input-sm"
                      placeholder="搜尋"
                      v-model="filters.search"
                      @keydown.enter="getPromotions"
      ...
      <div class="col-sm-12">
        <el-table v-loading="loading"
          ...
          <el-table-column label="推賡名稱" sortable prop="name" min-width="200"></el-table-column>
          <el-table-column label="顯示於" sortable prop="type" min-width="200">
            <template #default="scope">{{ promotionTypes[scope.row.type] }}</template>
          </el-table-column>
          ...
          <el-table-column class-name="action-buttons td-actions" label="" min-width="110">
            <template #default="scope">
              <p-button type="success" icon @click.stop="openPromotionDetail(scope.row.id)">
                <i class="fa fa-edit" aria-hidden="true"></i>
              </p-button>

              <p-button type="success" icon @click.stop="deletePromotion(scope.row.id)">
    ...
    <PromotionDetail ref="PromotionDetail"></PromotionDetail>
...
export default {
  ...
  methods : {
    ...
    async openPromotionDetail (promotionId) {
      const result = await this.$refs.PromotionDetail.open(promotionId)
      if (result) {
        //DO STH
        this.getPromotions()
      }
    ...
    async deletePromotion (promotionId) {
      try {
        ...
        if (confirmation.isConfirmed) {
          const result = await this.$api.delete('/v1/promotions/' + promotionId)
          if (result) {
            this.getPromotions()
          }
        }
    ...
    async changeActive (promotionId, val) {
      const loading = new Loading.service({ fullscreen : false });
      try {
        const result = await this.$api.patch('/v1/promotions/' + promotionId, { publish : val })
        if (result) {
    ...
    debounceData : _.debounce(async function () {
      ...
      try {
        ...
        const promotions = await t.$api.get('/v1/promotions', { params })
        if (promotions && promotions.data) {
          t.result = promotions.data
        }
      } catch (e) {
        console.log(e)
      } finally {
  ...
  data () {
    return {
      ...
      promotions     : [{ id : '', name : { zh : '全部', en : 'all' } }],
      loading        : false,
