    ...
    for (let orderIndex in orders) {
      ...
      _.forOwn(grouped, function (products, catName) {
        products.forEach(item => {
          if ((catName === '甜品' || catName === '奶粉') && length % 2 == 0 && length > 1) {
            print_y = y
            print_x += 24
          } else if (catName === '雲吞' && length % 1 == 0 && length > 0) {
            print_y = y
            print_x += 24
      ...
      if (order.prize && order.prize.type === 'item') {
        doc.setFontSize(15)
        doc.text(`送${ order.prize.product.internalName }x${ order.prize.amount }`, 60, max_y - 24)
      ...
      if (order.gift) {
        ...
        if (order.gift.type === 'item') {
          doc.text(`送${ order.gift.product.internalName }x${ order.gift.amount }`, 60, max_y - 10)
        } else {
          doc.text(`送${ order.gift.giftRef.name.zh.substring(0, 3) }x${ order.gift.amount }`, 60, max_y - 10)
        }
